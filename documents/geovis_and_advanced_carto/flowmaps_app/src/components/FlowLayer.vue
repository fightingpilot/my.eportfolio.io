...
// import * as L from 'src/components/js/LeafletCanvasFlowmapLayer/CanvasFlowmapLayer.js'
// import L from 'leaflet.canvas-flowmap-layer'
// import {Wrapper_CanvasFlowmapLayer} from "@/components/js/Wrapper_CanvasFlowmapLayer";y
import FileDropper from "./FileDropper"
// import {Tween} from "@tweenjs/tween.js";
// import TWEEN from "tween";
import {Wrapper_CanvasFlowmapLayer} from "@/components/js/Wrapper_CanvasFlowmapLayer";
//import {parseCSV} from "@/components/js/CanvasFlowmapLayer";
...
export default {
  ...
  data: function () {
    return {
      W_CFL: null,
      // flowMarker: [L.latLng(64.123780, -21.926925), L.latLng(65.123780, -21.926925)],
      options: {
  ...
  mounted() {
    // let self = this;
    // W_CFL = new Wrapper_CanvasFlowmapLayer(this.$window);
    this.W_CFL = new Wrapper_CanvasFlowmapLayer(this.$window);
    /**
    console.log(window.L.CanvasFlowmapLayer);
    var canvasRenderer = L.canvas();

    L.GeoJSON.CanvasFlowmapLayer = L.GeoJSON.extend({
      options: {
      ...
      initialize: function(geoJson, options) {
        ...
        if (Object.hasOwnProperty.call(window, 'TWEEN')) {
          console.log('Teen is available');
          ...
          // this._animationTween = new Tween(this._animationPropertiesStatic)
              .to({
      ...
      setOriginAndDestinationGeoJsonPoints: function(geoJsonFeatureCollection) {
        ...
        if (geoJsonFeatureCollection.features) {
          ...
          // console.log(geoJsonPointsToDraw);
      ...
      setAnimationEasing: function(easingFamily, easingType) {
        ...
        if (
            // Object.Easing.hasOwnProperty.call(easingFamily, 'TWEEN') &&
            TWEEN.Easing.hasOwnProperty.call(easingFamily) &&
            // Object.Easing[easingFamily].hasOwnProperty.call(easingType, 'TWEEN')
            TWEEN.Easing[easingFamily].hasOwnProperty.call(easingType)
        ) {
          // tweenEasingFunction = Object.Easing[]
    ...
    });// end of setting up CanvasFlowmapLayer

    L.GeoJSON.canvasFlowmapLayer = function (geoJson) {
      console.log("Create CanvasFlowmapLayer");
      console.log(geoJson);
      // return new L.GeoJSON(geoJson);
      return new L.GeoJSON.CanvasFlowmapLayer(geoJson);
      // return null;
    }**/
  },
  methods: {
    loadFromFileUpload(file) {
      var self = this;
      // console.log(file);
      if (file.name.split(".").at(-1).toLowerCase() === "csv") {
        let reader = new FileReader();
        reader.onload = function(e) {
          // self.csvToGeojson(e.target.result);
          // self.layerFromCsv(e.target.result);
          self.createFlowMapLayerFromCSV(e.target.result);
        }
        reader.readAsText(file);
      }
    },
    async createFlowMapLayerFromCSV(csv) {
      let self = this;
      console.log("createFlowMapLayerFromCSV");
      let p = this.W_CFL.parseCSV(csv)
      let layer = p.then(function (value) {
        console.log(value);
        return self.W_CFL.createCanvasFlowMapLayer(value);
      });
      console.log(layer);
      self.$parent.addLayer(layer);
    }
  }
}
</script>
